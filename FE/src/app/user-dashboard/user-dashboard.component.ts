  ...
  export class UserDashboardComponent implements OnInit {
    // 'none' là đang đóng. Các giá trị khác tương ứng với 4 ô vuông.
...
loadMyGroup() {
    ...
    this.groupService.getMyGroup().subscribe({
      next: (res) => {
        this.myGroup = res; // Cập nhật biến myGroup chứa MentorId và MentorName
 ...
 handleCreateGroup() {
    ...
    this.groupService.createGroup({
      ...
      next: (res) => {
        this.isLoading = false; // Phải tắt loading
        this.successMessage = res.message;
        this.newGroupName = '';
        this.loadMyGroup(); // Cập nhật lại myGroup để các nút chuyển trạng thái
      },
      error: (err) => {
